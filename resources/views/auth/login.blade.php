...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Iniciar sesión | FantaSync</title>
            ...
            <article class="auth-card__content">
                <header class="auth-header">
                    ...
                    <hgroup class="auth-card__brand">
                        ...
                        <h1 class="auth-card__title">FantaSync</h1>
                        <p class="auth-card__description">Ingresa tus credenciales para acceder al panel de administración.</p>
                    </hgroup>
                </header>

                <form method="POST" action="{{ route('login.submit') }}" novalidate class="auth-form">
                    @csrf
                    ...
                    @if(session('error'))
                        <p class="form-error form-error--global">{{ session('error') }}</p>
                    @endif

                    <footer class="form-actions">
                        <button type="submit" class="button-submit">Iniciar sesión</button>
                        
                        <span class="divider" aria-hidden="true">o</span>

                        <a href="{{ route('google.redirect') }}" class="button-google" rel="nofollow">
