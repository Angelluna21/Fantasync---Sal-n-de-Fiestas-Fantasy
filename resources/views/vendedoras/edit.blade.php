...
<head>
    ...
    <title>Editar Vendedora · FantaSync</title>
    ...
    <main class="dashboard-layout">
        ...
        <header class="dashboard-header">
            <hgroup>
                ...
                <h1 class="dashboard-title">Editar Vendedora</h1>
                <p class="dashboard-description">Actualiza la información de la vendedora.</p>
            </hgroup>
        </header>

        <!-- Formulario -->
        <section class="form-section" aria-label="Formulario de edición de vendedora">
            <section class="form-container">
                ...
                <form method="POST" action="{{ route('vendedoras.update', $vendedora->id) }}" class="form-card">
                    @csrf
                    @method('PUT')
                    ...
                    <fieldset class="form-group">
                        ...
                        <input 
                            ...
                            value="{{ old('telefono', $vendedora->telefono) }}"
                        >
                        @error('telefono')
                            <span class="form-error">{{ $message }}</span>
                        @enderror
                    ...
                    <fieldset class="form-group">
                        ...
                        <input 
                            ...
                            value="{{ old('email', $vendedora->email) }}"
                        >
                        @error('email')
                            <span class="form-error">{{ $message }}</span>
                        @enderror
                    ...
                    <fieldset class="form-group">
                        ...
                        <select 
                            ...
                            required
                        >
                            <option value="activo" {{ old('estado', $vendedora->estado) === 'activo' ? 'selected' : '' }}>🟢 Activa</option>
                            <option value="inactivo" {{ old('estado', $vendedora->estado) === 'inactivo' ? 'selected' : '' }}>🔴 Inactiva</option>
                        </select>
                        @error('estado')
                            <span class="form-error">{{ $message }}</span>
                        @enderror
